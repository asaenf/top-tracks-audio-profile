...
public class HttpTopTracksProvider implements TopTracksProvider {
    ...
    public List<String> getTopTracksIds(final RequestContext requestContext, final String token) {
        ...
        final CompletionStage<Response<ByteString>> responseFuture =
                requestContext.requestScopedClient().send(Request.forUri(topTracksUrl, "GET").withHeader("Authorization:", "Bearer " + token));
